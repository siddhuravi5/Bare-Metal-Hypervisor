...
{
	/*MTRR (Memory Type Range Register) - They are used to respect the cache policies of RAM segments in EPT*/
	...
	DbgPrint("VPID : 0x%llx\n", VpidRegister.Flags);
	DbgPrint("MTRR : 0x%llx\n", MTRRDefType.Flags);
	
	if (VpidRegister.PageWalkLength4 == 0)
	{
		LogWarning("VpidRegister.PageWalkLength4 error");
		//return FALSE;
	}
	if (VpidRegister.MemoryTypeWriteBack == 0)
	{
		LogWarning("VpidRegister.MemoryTypeWriteBack error");
		//return FALSE;
	}
	if (VpidRegister.Pde2MbPages == 0)
	{
		LogWarning("VpidRegister.Pde2MbPages error");
		//return FALSE;
	}
	if (VpidRegister.AdvancedVmexitEptViolationsInformation == 0)
	{
		LogWarning("The processor doesn't report advanced VM-exit information for EPT violations");
	}

	if (MTRRDefType.MtrrEnable == 0)
	{
		LogWarning("Mtrr Dynamic Ranges not supported");
		//return FALSE;
	}
...
BOOLEAN EptBuildMtrrMap()
{
	...
	DbgPrint("MTRR variable range count : 0x%llx\n", MTRRCap.VariableRangeCount);
...
{
	//index is the page frame number bcaz of identity mapping
	...
	//my modification code for helping
	if (EptState->NumberOfEnabledMemoryRanges == 0) {
		DbgPrint("Cache Type : MEMORY_TYPE_UNCACHEABLE\n");
...
BOOLEAN EptHandlePageHookExit(VMX_EXIT_QUALIFICATION_EPT_VIOLATION ViolationQualification, UINT64 GuestPhysicalAddr)
{
	...
	*/
	...
	if (!ViolationQualification.EptExecutable && ViolationQualification.ExecuteAccess)
	{
		...
		AddToSecretData("Set the Execute Access of a page to 1, where PFN = 0x", TargetPage->PageFrameNumber);
...
{
	DbgPrint("Handling EPT Violation\n");
	//DbgBreakPoint();
...
BOOLEAN EptVmxRootModePageHook(PVOID TargetFunc, BOOLEAN HasLaunched) {
	...
	// the virtual target points to the starting address of 4K page which contains the target function definition.
...
BOOLEAN EptPageHook(PVOID TargetFunc, BOOLEAN HasLaunched) {
	DbgPrint("Applying EPT Page Hook\n");
	ULONG LogicalCoreIndex;
	PVOID PreAllocBuff;
	LogicalCoreIndex = KeGetCurrentProcessorIndex();
	...
	if (HasLaunched && !VMX_ROOT)
	{
		if (AsmVmxVmcall(VMCALL_EXEC_HOOK_PAGE, TargetFunc, NULL, NULL, NULL) == STATUS_SUCCESS)
		{
			...
			BMHV_Notify_All_To_Invalidate_Ept();
		...
		if (EptVmxRootModePageHook(TargetFunc, HasLaunched) == TRUE) {
			LogInfo("Hook applied (launched VM)");
			return TRUE;
		}
	}
	else
